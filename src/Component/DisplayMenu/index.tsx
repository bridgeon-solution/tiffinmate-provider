...
export const MenuDisplay: React.FC<MenuDisplayProps> = ({ menuList, foodItems, loading, error }) => {
  if (loading) {
    return (
                  ...
                  <Box key={food.food_name} sx={{ minWidth: 250, marginRight: 2 }}>
                    <Card sx={{ display: 'flex', flexDirection: 'column', padding: 1, borderRadius: 2, height: 250, width: '100%', boxShadow: 4 }}>
                      ...
                      <CardContent sx={{ height: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        <Typography variant="h6" gutterBottom sx={{ fontSize: "0.85rem", color: "primary.main", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.5px" }}>
                          {food.day}
                        </Typography>

                        <Typography variant="h6" sx={{ fontSize: "1rem", fontWeight: "bold", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {food.food_name}
                        </Typography>

                        <Typography variant="subtitle1" sx={{ mt: 1, fontSize: "0.9rem", fontWeight: "700", marginTop: 0, color: "black" }}>
                          ₹{food.price}
                        </Typography>
                      </CardContent>
                    </Card>
